//===============================================================================
function LCDisplay(ctx, xpos, ypos, scale, chars, lines, textcolor)
{
  this.ctx = ctx;
  this.xpos = xpos;
  this.ypos = ypos;
  ...
  this.dw = 6 * scale;  //Scaled single display width
  this.dh = 9 * scale;  //Scaled single display width
  this.cw = 5 * scale;  //Scaled character width
  this.ch = 8 * scale;  //Scaled character height

  this.width = (chars * 6 * scale) + 11;
  this.height = (lines * 9 * scale) + 11;

  this.drawdisplay();
}

//-----------------------------------------------------------
// char and line start on 0
//-----------------------------------------------------------
LCDisplay.prototype.displaytext = function(char, line, text)
{
  var ctx = this.ctx;
  var nc = this.chars * this.lines;
  var p = (line * this.chars) + char;
  var x = this.xpos + 6 + (char * this.dw);
  var y = this.ypos + 6 + (line * this.dh);
  var w = this.xpos + this.width - 6;
  var l = text.length;
  var i;
  ...
  for(i=0;i<l;i++)
  {
    this.drawcharacter(x,y,text.charCodeAt(i));
...
//-----------------------------------------------------------
// Draw the pixels for a single character
//-----------------------------------------------------------
LCDisplay.prototype.drawcharacter = function(xpos, ypos, char)
{
  //Each character has 8 rows and per row 5 bits are used
  var ctx = this.ctx;
  var scale = this.scale;
  var rows = this.Character[char & 0xFF];
  var col;
  var ri,ci;
  var x = xpos;

  //Refresh the green background
  ctx.fillStyle = "#88C800";
  ctx.fillRect(x-1,ypos-1,this.cw+2,this.ch+2);

  //Refresh the mask to delete previous characters
  ctx.fillStyle = "#78B000";
  ctx.fillRect(x, ypos, this.cw, this.ch);

  ctx.fillStyle = this.textcolor;

  //Plot scale sized square pixels
  for(ri=0;ri<8;ri++)
  {
    col = rows[ri];
    ...
    for(ci=0;ci<5;ci++)
    {
      ...
      if(col & 0x100)
        ctx.fillRect(x, ypos, scale, scale);
...
//-----------------------------------------------------------
// Draw the basic layout of a display
//-----------------------------------------------------------
LCDisplay.prototype.drawdisplay = function()
{
  var ctx = this.ctx;
  ...
  //Left top edge with a dark color for depth
  ctx.beginPath();
  ctx.moveTo(x1,y2);
  ctx.lineTo(x1,y1);
  ctx.lineTo(x2,y1);
  ...
  //bottom right edge with a light color also for depth
  ctx.beginPath();
  ctx.moveTo(x2,y1);
  ctx.lineTo(x2,y2);
  ctx.lineTo(x1,y2);
  ...
  //The masks for the separate characters
  ctx.fillStyle = "#78B000";

  for(l=0;l<this.lines;l++)
  {
    for(c=0;c<this.chars;c++)
    {
      ctx.fillRect(x1, y1, this.cw, this.ch);
...
//-----------------------------------------------------------
// Bit patterns for drawing the characters
//-----------------------------------------------------------
LCDisplay.prototype.Character =
[
   ...
   [ 0x70, 0x88, 0x80, 0xB8, 0x88 ,0x88, 0x78, 0x00 ],    //0x47
   ...
   [ 0x88, 0x88, 0x88, 0xA8, 0xA8 ,0xA8, 0x50, 0x00 ],    //0x57
   ...
   [ 0x00, 0x78, 0x88, 0x88, 0x78 ,0x08, 0x70, 0x00 ],    //0x67
   ...
   [ 0x00, 0x20, 0x40, 0xF8, 0x40 ,0x20, 0x00, 0x00 ],    //0x7F
   ...
   [ 0x00, 0x00, 0x00, 0x00, 0xE0 ,0xA0, 0xE0, 0x00 ],    //0xA1
   ...
   [ 0x00, 0x00, 0x00, 0x60, 0x60 ,0x00, 0x00, 0x00 ],    //0xA5
   [ 0x00, 0x00, 0x00, 0x00, 0x00 ,0x00, 0x00, 0x00 ],    //0xA6  Tot hier ingevuld
   ...
   [ 0xF8, 0xF8, 0xF8, 0xF8, 0xF8 ,0xF8, 0xF8, 0xF8 ]     //0xFF
];
